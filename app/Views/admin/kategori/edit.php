...
<div class="row mb-4">
    <div class="col-12">
        ...
        <p class="text-muted">Perbarui data kategori pengaduan</p>
    </div>
</div>
...
<div class="row">
    <div class="col-lg-8">
        <div class="card shadow">
            ...
            <div class="card-body">
                <form method="post" action="<?= base_url('admin/kategori/' . $kategori['id']) ?>">
                    <?= csrf_field() ?>
                    <div class="mb-3">
                        <label class="form-label">Nama Kategori <span class="text-danger">*</span></label>
                        <input type="text" name="nama" class="form-control <?= isset($errors['nama']) ? 'is-invalid' : '' ?>" value="<?= old('nama', $kategori['nama']) ?>" required>
                        <?php if(isset($errors['nama'])): ?><div class="invalid-feedback"><?= $errors['nama'] ?></div><?php endif; ?>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Deskripsi</label>
                        <textarea name="deskripsi" class="form-control <?= isset($errors['deskripsi']) ? 'is-invalid' : '' ?>" rows="3"><?= old('deskripsi', $kategori['deskripsi']) ?></textarea>
